...
<span class="brand">protokoll</span> is an EC-VRF oracle for EVM chains - verifiable randomness built from first principles.

The name comes from German. It means *protocol*. But the reference is more specific than that.
...
Neurath and Carnap debated whether protocol sentences described inner experience or physical states. They disagreed on the details. Both agreed on the core: knowledge must bottom out in something checkable.
...
The word carries a second meaning that arrives for free: blockchain infrastructure. Every smart contract system runs on a protocol - a set of rules that determines which state transitions are valid. <span class="brand">protokoll</span> names both layers simultaneously:
